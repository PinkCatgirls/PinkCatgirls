...
class Base {
    ...
    toJSON() {
        ...
        for (let prop in newRels) {
            ...
            if (value && !Array.isArray(value)) {
                x.links = {
                    related: `https://www.patreon.com/api/${Base.resolveApiEndpoint(value.type)}/${value.id}`
                };
            }
    ...
    static resolveClass(hub, data) {
        ...
        if (patreonTypes.includes(className)) {
            ...
            if (relevantClass)
                return new relevantClass(hub, data);
        }
        else
            return new Base(hub, data);
    }
    fetch(cache = true) {
            ...
            return new Promise((resolve, reject) => {
                https.get(this.link, res => {
                    ...
                    res.on('end', () => {
                        ...
                        if (cache)
                            return resolve(this.hub.parseApiResponse(fetched));
                        else
                            return resolve(Base.resolveClass(fetched.data));
                    });
                });
            });
        });
