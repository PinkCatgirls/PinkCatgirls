...
class Util {
    ...
    static verifyPatreonIdentity(req, secret) {
        ...
        const hexDigest = hmac.update(JSON.stringify(req.body)).digest('hex');
        console.log("hexDigest", hexDigest, req.headers['x-patreon-signature']);
        return true;
        if (hexDigest === req.headers['x-patreon-signature'])
            return true;
        else
            return false;
    }
